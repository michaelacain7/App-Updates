...
def main():
    ...
    parser.add_argument("--interval", type=int, default=CHECK_INTERVAL_MINUTES, help="Minutes between scans")
    parser.add_argument("--monitor", type=str, help="Run specific monitor only (js,docs,sitemaps,github,appstore,playstore,chrome,subdomains,pages)")
    ...
    if args.once:
        ...
        logger.info(f"Starting continuous monitoring (every {args.interval} min)")
        while True:
            try:
                run_all_monitors(config, conn)
            except Exception as e:
