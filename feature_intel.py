...
def monitor_docs(conn, targets):
    ...
    Only alerts when changes contain actual product/feature signals,
    not routine text edits or formatting changes.
    """
    logger.info("=== Documentation Monitor ===")

    # Patterns that indicate real product/feature signals in doc changes
    DOC_SIGNAL_PATTERNS = [
        # New features / products
        r"\bnew\b", r"\blaunch", r"\breleased?\b", r"\bannounce", r"\bintroduc",
        r"\bnow available\b", r"\bnow support", r"\bcoming soon\b",
        r"\brolling out\b", r"\bgenerally available\b", r"\bGA\b",
        r"\bbeta\b", r"\bpreview\b", r"\bexperimental\b", r"\bearly access\b",
        # Access control / permissions / security
        r"\bRBAC\b", r"\baccess control\b", r"\bpermission", r"\brole-based\b",
        r"\bSSO\b", r"\bSAML\b", r"\bOAuth\b", r"\bMFA\b", r"\b2FA\b",
        # API / Integration
        r"\bnew API\b", r"\bendpoint", r"\bwebhook", r"\bSDK\b",
        r"\bintegration\b", r"\bconnector\b", r"\bplugin\b", r"\bextension\b",
        # Architecture / Infrastructure
        r"\bmigrat", r"\bdeprecated?\b", r"\bsunset", r"\bend.of.life\b",
        r"\bbreaking change\b", r"\bbackward",
        # Pricing / Plans
        r"\bpric", r"\bplan\b", r"\btier\b", r"\bfree\b", r"\benterprise\b",
        r"\bpro\b", r"\bpremium\b", r"\bsubscription\b",
        # AI / Model
        r"\bmodel\b", r"\bagent\b", r"\bLLM\b", r"\bGPT\b", r"\bgemini\b",
        r"\bclaude\b", r"\bcopilot\b", r"\bAI\b",
        # Limits / Quotas
        r"\blimit", r"\bquota\b", r"\brate.limit\b", r"\bthrottle\b",
        r"\bcapacity\b", r"\bscaling\b",
        # Data
        r"\bregion\b", r"\bcompliance\b", r"\bGDPR\b", r"\bSOC\b", r"\bHIPAA\b",
        r"\bencrypt", r"\bprivacy\b",
    ]

    DOC_SIGNAL_RE = re.compile("|".join(DOC_SIGNAL_PATTERNS), re.IGNORECASE)

    # Noise patterns to ignore
    DOC_NOISE_PATTERNS = re.compile(
        r"^\d{4}[-/]\d{2}[-/]\d{2}$"           # Date-only lines
        r"|^(january|february|march|april|may|june|july|august|september|october|november|december)\s+\d"
        r"|^updated?\s*(on|:)?\s*\d"            # "Updated on 2026-03-01"
        r"|^last\s*(modified|updated|edited)"    # "Last modified"
        r"|^copyright\s"                         # Copyright lines
        r"|^©\s"
        r"|^\s*\d+\s*$"                          # Just a number
        r"|^(home|about|contact|blog|docs|faq|help|terms|privacy)\s*$"  # Nav items
    , re.IGNORECASE)
    ...
    for target in targets:
        ...
        for url in urls:
            ...
            if old_hash and content_hash != old_hash:
                ...
                # Filter to only signal-bearing lines
                signal_lines = []
                for line in added:
                    ...
                    if len(line_stripped) < 10:
                        continue
                    if DOC_NOISE_PATTERNS.search(line_stripped):
                        continue

                    # Keep if matches a signal pattern OR a user-defined keyword
                    if DOC_SIGNAL_RE.search(line_stripped):
                        signal_lines.append(line)
                    elif keywords and any(kw in line_stripped.lower() for kw in keywords):
                        signal_lines.append(line)

                if signal_lines:
                    sample = "\n".join(f"+ {line[:150]}" for line in signal_lines[:10])
                    send_discord_alert(
                        f"Doc Change: {name}",
                    ...
                    logger.debug(f"  Minor doc change (no signals): {name} ({len(added)} lines)")
