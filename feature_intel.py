...
def get_play_store_info(package_name):
    """
    Scrape Google Play Store for app version and changelog.
    Google Play embeds all data in AF_initDataCallback JSON blobs.
    """
    ...
    # Primary method: Extract ALL strings from AF_initDataCallback data blobs
    # These contain the actual app data including changelogs
    all_data_strings = []
    for match in re.finditer(r"AF_initDataCallback\(\{.*?data:(.*?)\}\s*\)\s*;", html, re.DOTALL):
        data_str = match.group(1)[:100000]
        # Extract all quoted strings from the data
        strings = re.findall(r'"((?:[^"\\]|\\.){10,2000})"', data_str)
        all_data_strings.extend(strings)

    # Find changelog: look for the "What's new" content
    # It's typically a longer descriptive string near version-like strings
    for s in all_data_strings:
        s_clean = s.replace("\\n", "\n").replace("\\t", " ").replace('\\"', '"')
        # Changelog indicators: multi-sentence, mentions bugs/features/improvements
        s_lower = s_clean.lower()
        if any(kw in s_lower for kw in [
            "bug fix", "improve", "fix", "update", "feature", "new",
            "enhance", "change", "performance", "stabil", "patch",
            "added", "removed", "resolved", "addressed", "optimiz",
            "we've", "this version", "this release", "what's new",
            "minor", "major", "introducing", "now you can",
        ]):
            # Must be at least somewhat long and not be the app description
            if 20 < len(s_clean) < 2000:
                # Prefer shorter changelog-style text over long descriptions
                if not whats_new or len(s_clean) < len(whats_new):
                    whats_new = s_clean

    # Find version number from the data strings
    for s in all_data_strings:
        v_match = re.match(r'^(\d+\.\d+[\.\d]*)$', s.strip())
        if v_match and len(s) <= 20:
            version = v_match.group(1)
            break

    # Fallback: try meta/HTML extraction
    if not version:
        soup = BeautifulSoup(html, "html.parser")
        for div in soup.find_all(string=re.compile(r"^\d+\.\d+[\.\d]*$")):
            version = div.strip()
            break

    # Fallback changelog: look for "What's New" section in HTML
    if not whats_new:
        soup = BeautifulSoup(html, "html.parser") if 'soup' not in dir() else soup
        for heading in soup.find_all(string=re.compile(r"What.?s [Nn]ew", re.IGNORECASE)):
            parent = heading.find_parent()
            if parent:
                container = parent.find_parent("div") or parent.find_parent("section")
                if container:
...
def monitor_play_store(conn, targets):
    ...
    for i, target in enumerate(targets, 1):
        ...
        if old_hash and info["html_hash"] != old_hash:
            ...
            changelog = info['whats_new'] if info['whats_new'] else "No changelog available"
            send_discord_alert(
                f"Play Store Update: {name}",
                f"**`{package}`** has been updated\n\n{version_str}**What's New:**\n{changelog[:1500]}",
                fields=[
